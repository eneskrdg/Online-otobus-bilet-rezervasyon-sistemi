...
class Report_Model extends Model {
    ...
    public function getBookerrData($journeyDate, $busNo) {
        return $this->db->select('SELECT
                                    booking.bookingID,
                                    booking.bookerTCNo,
                                    (SELECT bookerName FROM booker WHERE bookerTCNo = booking.bookerTCNo) AS bookerName,
                                    (SELECT entryPoint FROM entry_point WHERE entryPointNo = booking.entryPointNo) AS entryPointNo,
                                    booking.no_of_seat,
                                    booking.ammount,
                                    (SELECT bookerMNo FROM booker WHERE bookerTCNo = booking.bookerTCNo) AS bookerMNo
                                    FROM booking 
                                    WHERE booking.busNo ="' . $busNo . '" AND booking.date ="' . $journeyDate . '" AND booking.payment_status ="Ok" ');
    }
    ...
    public function searchBookingData($journeyDate, $busNo, $journeyNo) {
        ...
        $resultBookingSeat = $this->db->select('SELECT 
                                                booking.bookingID,
                                                (SELECT bookerMNo FROM booker WHERE booker.bookerTCNo = booking.bookerTCNo) AS bookerMNo,
